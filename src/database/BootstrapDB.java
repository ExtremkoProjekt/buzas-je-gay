    ...
    private static void fillBuildingProgress() throws SQLException, ClassNotFoundException {
        ...
        if (BuildingProgressRepository.isEmpty()) {
            ...
            for (int i = 1; i <= numberOfLevels; i++) {
                BuildingProgressRepository.insert(new BuildingProgress(1, i, i * 15, i * i * 5, i));
            }
            ...
            for (int i = 1; i <= numberOfLevels; i++) {
                if (i == numberOfLevels) {
                    BuildingProgressRepository.insert(new BuildingProgress(3, i, i * 20, 1, i));
                } else { // ak je Hlavna budova == level 5, value zvys na 1 (inak 0)
                    BuildingProgressRepository.insert(new BuildingProgress(3, i, i * 20, 0, i));
                }
            }
        }
    }
